...
def get_source_and_test_files(path):
    test_dict = {}
    source_dict = {}
    python_files = get_py_files(path)
...
def run_bugbane(parser):
    ...
    if config.source_file and config.test_file:
        ...
        source_dict, test_dict = get_source_and_test_files(source_folder_path)
        ...
        file_names = list(source_dict.keys())
        for original_filename in file_names:
            test_filename = test_dict[original_filename]
            if test_filename:
                mutants, html_diff = apply_mutations_to_file(source_dict[original_filename],mutation_operators,config.num_mutants)
                number_of_mutants = len(mutants)
                total_number_of_mutants += number_of_mutants
                copied_file = create_file_copy(source_dict[original_filename])  
                if not config.py_test:
                    for index in range(number_of_mutants):
                        load_file(source_dict[original_filename],mutants[index])
                        if run_file_against_tests(source_dict[original_filename],test_dict[original_filename],True):
                    ...
                    for index in range(number_of_mutants):
                        load_file(source_dict[original_filename],mutants[index])
                        if run_file_against_tests(source_dict[original_filename],test_dict[original_filename],False):
                            number_of_test_passed += 1
                        else:
                            number_of_test_failed += 1
                ...
                os.remove(source_dict[original_filename][:-3] + '-copy.py')
