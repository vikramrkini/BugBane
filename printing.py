...
try:
    ...
    try:
        ...
        pydot_imported_msg = 'Install the python package pydot or pydot-ng. Install graphviz.'
...
def debugprint(obj, depth=-1, print_type=False, file=None, ids='CHAR', stop_on_name=False, done=None, print_storage=False, print_clients=False, used_ids=None):
    ...
    for (r, p, s, o) in zip(results_to_print, profile_list, smap, order):
        if hasattr(r.owner, 'op') and isinstance(r.owner.op, theano.scan_module.scan_op.Scan):
            scan_ops.append(r)
        debugmode.debugprint(r, depth=depth, done=done, print_type=print_type, file=_file, order=o, ids=ids, scan_ops=scan_ops, stop_on_name=stop_on_name, profile=p, smap=s, used_ids=used_ids, print_clients=print_clients)
    if len(scan_ops) > 0:
        print('', file=_file)
        new_prefix = ' >'
        new_prefix_child = ' >'
        ...
        for s in scan_ops:
            ...
            inner_to_outer_inputs = dict([(inner_inputs[i], outer_inputs[o]) for (i, o) in s.owner.op.var_mappings['outer_inp_from_inner_inp'].items()])
            print('', file=_file)
            debugmode.debugprint(s, depth=depth, done=done, print_type=print_type, file=_file, ids=ids, scan_ops=scan_ops, stop_on_name=stop_on_name, scan_inner_to_outer_inputs=inner_to_outer_inputs, print_clients=print_clients, used_ids=used_ids)
            if hasattr(s.owner.op, 'fn'):
            ...
            for (idx, i) in enumerate(outputs):
                if hasattr(i, 'owner') and hasattr(i.owner, 'op'):
                    if isinstance(i.owner.op, theano.scan_module.scan_op.Scan):
                        scan_ops.append(i)
                debugmode.debugprint(r=i, prefix=new_prefix, depth=depth, done=done, print_type=print_type, file=_file, ids=ids, stop_on_name=stop_on_name, prefix_child=new_prefix_child, scan_ops=scan_ops, scan_inner_to_outer_inputs=inner_to_outer_inputs, print_clients=print_clients, used_ids=used_ids)
    if file is _file:
        return file
    elif file == 'str':
    ...
    def process(self, output, pstate):
        ...
        def pp_process(input, new_precedence):
            try:
                ...
                pstate.precedence = old_precedence
            return r
        d = dict(((str(i), x) for (i, x) in enumerate((pp_process(input, precedence) for (input, precedence) in zip(node.inputs, precedences)))))
        r = pattern % d
        pstate.memo[output] = r
        return r
...
class FunctionPrinter:
    ...
    def process(self, output, pstate):
        ...
        try:
            old_precedence = getattr(pstate, 'precedence', None)
            pstate.precedence = new_precedence
            r = '%s(%s)' % (name, ', '.join([pprinter.process(input, pstate) for input in node.inputs]))
        finally:
            pstate.precedence = old_precedence
        pstate.memo[output] = r
        return r
...
class DefaultPrinter:
    ...
    def process(self, output, pstate):
        ...
        try:
            old_precedence = getattr(pstate, 'precedence', None)
            pstate.precedence = new_precedence
            r = '%s(%s)' % (str(node.op), ', '.join([pprinter.process(input, pstate) for input in node.inputs]))
        finally:
            pstate.precedence = old_precedence
        pstate.memo[output] = r
        return r
...
class PPrinter:
    ...
    def process_graph(self, inputs, outputs, updates=None, display_inputs=False):
        ...
        if display_inputs:
            ...
            strings = []
        pprinter = self.clone_assign(lambda pstate, r: r.name is not None and r is not current, leaf_printer)
        inv_updates = dict(((b, a) for (a, b) in iteritems(updates)))
        i = 1
        for node in gof.graph.io_toposort(list(inputs) + updates.keys(), list(outputs) + updates.values()):
...
if use_ascii:
    ...
    greek = dict(alpha='\\alpha', beta='\\beta', gamma='\\gamma', delta='\\delta', epsilon='\\epsilon')
else:
    special = dict(middle_dot=u'·', big_sigma=u'Σ')
    greek = dict(alpha=u'α', beta=u'β', gamma=u'γ', delta=u'δ', epsilon=u'ε')
pprint = PPrinter()
pprint.assign(lambda pstate, r: True, default_printer)
pp = pprint
'\nPrint to the terminal a math-like expression.\n'
default_colorCodes = {'GpuFromHost': 'red', 'HostFromGpu': 'red', 'Scan': 'yellow', 'Shape': 'brown', 'IfElse': 'magenta', 'Elemwise': '#FFAABB', 'Subtensor': '#FFAAFF', 'Alloc': '#FFAA22', 'Output': 'blue'}

def pydotprint(fct, outfile=None, compact=True, format='png', with_ids=False, high_contrast=True, cond_highlight=None, colorCodes=None, max_label_size=70, scan_graphs=False, var_with_name_simple=False, print_output_file=True, return_image=False):
    ...
    if outfile is None:
        outfile = os.path.join(config.compiledir, 'theano.pydotprint.' + config.device + '.' + format)
    ...
    if not pydot_imported:
        raise RuntimeError('Failed to import pydot. You must install graphviz and either pydot or pydot-ng for `pydotprint` to work.', pydot_imported_msg)
    g = pd.Dot()
    ...
    for (node_idx, node) in enumerate(topo):
        ...
        if use_color is None:
            ...
            nw_node = pd.Node(aid, label=astr, style='filled', fillcolor=use_color, shape=apply_shape)
        else:
        ...
        for (idx, var) in enumerate(node.inputs):
            ...
            if label:
                param['label'] = label
            if hasattr(node.op, 'view_map') and idx in reduce(list.__add__, node.op.view_map.values(), []):
                param['color'] = colorCodes['Output']
            elif hasattr(node.op, 'destroy_map') and idx in reduce(list.__add__, node.op.destroy_map.values(), []):
                param['color'] = 'red'
            if var.owner is None:
                ...
                if isinstance(var, SharedVariable):
                    color = 'cyan'
                if high_contrast:
                    g.add_node(pd.Node(varid, style='filled', fillcolor=color, label=varstr, shape=var_shape))
                else:
                    g.add_node(pd.Node(varid, color=color, label=varstr, shape=var_shape))
        ...
        for (idx, var) in enumerate(node.outputs):
            ...
            if out or var in input_update:
                ...
                if high_contrast:
                    g.add_node(pd.Node(varid, style='filled', label=varstr, fillcolor=colorCodes['Output'], shape=var_shape))
                else:
                    g.add_node(pd.Node(varid, color=colorCodes['Output'], label=varstr, shape=var_shape))
            elif len(var.clients) == 0:
                g.add_edge(pd.Edge(aid, varid, **param))
                if high_contrast:
                    g.add_node(pd.Node(varid, style='filled', label=varstr, fillcolor='grey', shape=var_shape))
                else:
                    g.add_node(pd.Node(varid, label=varstr, color='grey', shape=var_shape))
    ...
    if not outfile.endswith('.' + format):
        outfile += '.' + format
    if scan_graphs:
        scan_ops = [(idx, x) for (idx, x) in enumerate(topo) if isinstance(x.op, theano.scan_module.scan_op.Scan)]
        ...
        for (idx, scan_op) in scan_ops:
            ...
            pydotprint(to_print, new_name, compact, format, with_ids, high_contrast, cond_highlight, colorCodes, max_label_size, scan_graphs)
    if return_image:
        ...
        try:
            ...
            if version and [int(n) for n in version.split('.')] < [1, 0, 28]:
                raise Exception('Old version of pydot detected, which can cause issues with pydot printing. Try upgrading pydot version to a newer one')
            raise
...
def min_informative_str(obj, indent_level=0, _prev_obs=None, _tag_generator=None):
    ...
    if hasattr(obj, '__array__'):
        ...
        for ipt in obj.owner.inputs:
            name += '\n'
            name += min_informative_str(ipt, indent_level=indent_level + 1, _prev_obs=_prev_obs, _tag_generator=_tag_generator)
    else:
        name = str(obj)
    prefix = cur_tag + '. '
...
def var_descriptor(obj, _prev_obs=None, _tag_generator=None):
    ...
    if hasattr(obj, '__array__'):
        ...
        name = str(obj.owner.op) + '('
        name += ','.join((var_descriptor(ipt, _prev_obs=_prev_obs, _tag_generator=_tag_generator) for ipt in obj.owner.inputs))
        name += ')'
    elif hasattr(obj, 'name') and obj.name is not None:
        name = obj.name
    else:
